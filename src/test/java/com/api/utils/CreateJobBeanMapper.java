...
public class CreateJobBeanMapper {
	...
	@Step("Converting createJobBean to createJob payload  for create job API test")

	public static CreateJobPayload mapper(CreateJobBean bean) {
		LOGGER.info("Coverting create job bean {} to to create job payload", bean);
		...
		int modelId = Integer.parseInt(bean.getCustomer_product__product_id());
		...
		CreateJobPayload createJobPayload = new CreateJobPayload(mstServiceLocationId, mstPlatformId, warrantyStatusId,
				oemId, customer, customerAddress, customerProduct, problemList);
		
		LOGGER.info("Converted bean to payload {}", createJobPayload);
